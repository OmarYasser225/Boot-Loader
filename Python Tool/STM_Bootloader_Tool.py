...
def decode_menu_command_code(command):
    ...
    if(command  == 0 ):
        ...
        if(command == 7):
            sector_num = input("\n   Enter Page number(0-127 or 0xFF) here :")
        
        if(command== 6 or sector_num == "0xFF" or sector_num == "0xff" or sector_num == "FF" or sector_num == "ff"): 
            sector_num = 0xff
            nsec = 0
        else:
            sector_num = int(sector_num, 16)
            nsec=int(input("\n   Enter number of Sectors to erase(max 128) here :"))
...
def display_menu():
    ...
    # Display the title in a panel
    title_panel = Panel.fit(
        "    [bold yellow]STM32F103 BootLoader v2[/bold yellow]    ",
        border_style="green",
    ...
    menu_table = Table(title="Available Commands", style="bold blue")
    menu_table.add_column("Option", justify="center", style="bold cyan")
    menu_table.add_column("Command", justify="left", style="bold green")
    ...
    menu_items = [
        ...
        ("[4]", "BL_GET_RDP_STATUS"),
        ("[5]", "BL_GO_TO_ADDR"),
        ("[6]", "BL_FLASH_MASS_ERASE"),
        ("[7]", "BL_FLASH_ERASE"),
        ("[8]", "BL_MEM_WRITE"),
        ("[9]", "BL_Jump_To_User_App"),
        ("[10]", "BL_EN_R_W_PROTECT"),
        ("[11]", "BL_MEM_READ"),
        ("[12]", "BL_READ_SECTOR_P_STATUS"),
        ("[13]", "BL_OTP_READ"),
        ("[14]", "BL_DIS_R_W_PROTECT"),
        ("[15]", "BL_MY_NEW_COMMAND"),
        ("[0]", "MENU_EXIT"),
    ]

    for option, command in menu_items:
        menu_table.add_row(option, command)
...
def display_ports():
    ...
    if not ports:
        console.print(Panel("[bold red]No Ports are available.[/bold red]", border_style="red"))
        return False
    else:
        # Table for available devices
        table = Table(title="Available Devices", style="bold magenta")
        table.add_column("Port", justify="center", style="cyan", no_wrap=True)
        table.add_column("Description", justify="left", style="green")

        for port in ports:
            table.add_row(port.device, port.description)
